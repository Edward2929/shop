...
class SaveProductRequest extends Request
{
    ...
    public function getProductRules(): array
    {
        return array_merge(
            [
                ...
                'prices.*.currency' => ['required_with:prices.*.price', Rule::in(setting('supported_currencies'))],
                'prices.*.price' => 'nullable|numeric|min:0|max:99999999999999',
                'prices.*.special_price' => 'nullable|numeric|min:0|max:99999999999999',
                'prices.*.special_price_type' => ['nullable', Rule::in(['fixed', 'percent'])],
            ],
    ...
    public function getProductVariantsRules(): array
    {
        return [
            ...
            'variants.*.is_fixed_price' => 'sometimes|boolean',
            'variants.*.prices.*.currency' => ['required_with:variants.*.prices.*.price', Rule::in(setting('supported_currencies'))],
            'variants.*.prices.*.price' => 'nullable|numeric|min:0|max:99999999999999',
            'variants.*.prices.*.special_price' => 'nullable|numeric|min:0|max:99999999999999',
            'variants.*.prices.*.special_price_type' => ['nullable', Rule::in(['fixed', 'percent'])],
